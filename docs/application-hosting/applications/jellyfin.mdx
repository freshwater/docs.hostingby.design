...
Beware, some users have reported that some applications/versions of the jellyfin apps require a trailing slash to work, so if you are using the above url and are having connection issue then please append a / at the end of the url.
...
Connecting Jellyfin to services like Radarr or Sonarr for requires the Jellyfin port, which can be checked with the following command: `cat ~/.install/.jellyfin.lock`. 
